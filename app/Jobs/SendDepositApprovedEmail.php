...
class SendDepositApprovedEmail implements ShouldQueue
{
    ...
    public function handle(BrevoEmailService $brevoService): void
    {
        ...
        if (!$this->deposit || !$this->deposit->exists) {
            Log::error('Deposit not found for approved email job', [
        ...
        if (!$this->deposit->user) {
            Log::error('User not found for deposit approved email', [
                'deposit_id' => $this->deposit->id,
            ]);
        ...
        if (empty($this->deposit->user->email)) {
            Log::error('User email missing for deposit approved email', [
                'deposit_id' => $this->deposit->id,
        ...
        try {
            ...
            Log::info('Deposit approved email sent successfully', [
                'deposit_id' => $this->deposit->id,
                'user_id' => $this->deposit->user->id,
    ...
    {
        Log::critical('Deposit approved email job failed permanently', [
            'deposit_id' => $this->deposit->id,
            'user_id' => $this->deposit->user?->id,
