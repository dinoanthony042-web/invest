    ...
    <div id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-gray-900 transform -translate-x-full transition-transform duration-300 ease-in-out lg:translate-x-0">
        ...
        <nav class="mt-8 px-4">
            <div class="space-y-2">
                ...
                <a href="{{ route('transactions') }}" class="flex items-center px-4 py-3 text-sm font-medium text-white bg-yellow-400/20 rounded-lg border border-yellow-400/30">
            ...
            <div class="px-4 py-4">
                <div class="flex justify-between items-center">
                    <div class="flex items-center">
                        ...
                        <h1 class="text-xl font-bold text-yellow-400">Transactions</h1>
                    </div>
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
                <div class="bg-gray-800 rounded-3xl border border-yellow-400/20 p-6 shadow-lg">
                    <h2 class="text-lg font-semibold text-yellow-400 mb-4">Transaction Overview</h2>
                    <p class="text-gray-400 text-sm">View deposits, investments, and withdrawal history in one place.</p>
                </div>
                <div class="bg-gray-800 rounded-3xl border border-yellow-400/20 p-6 shadow-lg">
                    <h3 class="text-sm uppercase tracking-wide text-yellow-400 font-semibold mb-3">Deposit Requests</h3>
                    <p class="text-gray-300 text-sm">Total deposits: {{ $deposits->count() }}</p>
                </div>
                <div class="bg-gray-800 rounded-3xl border border-yellow-400/20 p-6 shadow-lg">
                    <h3 class="text-sm uppercase tracking-wide text-yellow-400 font-semibold mb-3">Withdrawal Requests</h3>
                    <p class="text-gray-300 text-sm">Total withdrawals: {{ $withdrawals->count() }}</p>
                </div>
            </div>

            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <section class="bg-gray-900 rounded-3xl border border-yellow-400/20 p-6">
                    <h2 class="text-xl font-semibold text-yellow-400 mb-4">Deposit History</h2>
                    <div class="space-y-4">
                        @forelse($deposits as $deposit)
                        <div class="rounded-3xl bg-gray-800 p-4 border border-yellow-400/10">
                            <div class="flex justify-between items-center mb-2">
                                <span class="text-white font-semibold">{{ ucfirst($deposit->network) }} deposit</span>
                                <span class="text-sm text-gray-400">{{ $deposit->created_at->diffForHumans() }}</span>
                            </div>
                            <p class="text-gray-400 text-sm">Amount: ${{ number_format($deposit->amount, 2) }}</p>
                            <p class="text-sm {{ $deposit->status === 'approved' ? 'text-green-400' : ($deposit->status === 'pending' ? 'text-yellow-400' : 'text-red-400') }}">Status: {{ ucfirst($deposit->status) }}</p>
                        </div>
                        @empty
                ...
                <section class="bg-gray-900 rounded-3xl border border-yellow-400/20 p-6">
                    <h2 class="text-xl font-semibold text-yellow-400 mb-4">Investment Purchases</h2>
                    <div class="space-y-4">
                        ...
                        <div class="rounded-3xl bg-gray-800 p-4 border border-yellow-400/10">
                            ...
                            <p class="text-sm {{ $investment->status === 'active' ? 'text-green-400' : 'text-gray-400' }}">Status: {{ ucfirst($investment->status) }}</p>
                        </div>
                        @empty
                        <p class="text-gray-500">No investment transactions yet.</p>
                        @endforelse
                    </div>
                </section>
            </div>

            <section class="mt-6 bg-gray-900 rounded-3xl border border-yellow-400/20 p-6">
                <h2 class="text-xl font-semibold text-yellow-400 mb-4">Withdrawal History</h2>
                <div class="space-y-4">
                    @forelse($withdrawals as $withdrawal)
                    <div class="rounded-3xl bg-gray-800 p-4 border border-yellow-400/10">
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-white font-semibold">{{ $withdrawal->investment->investmentPlan->name ?? 'Investment' }}</span>
                            <span class="text-sm text-gray-400">{{ $withdrawal->created_at->diffForHumans() }}</span>
                        </div>
                        <p class="text-gray-400 text-sm">Amount requested: ${{ number_format($withdrawal->amount, 2) }}</p>
                        <p class="text-sm {{ $withdrawal->status === 'approved' ? 'text-green-400' : ($withdrawal->status === 'pending' ? 'text-yellow-400' : 'text-red-400') }}">Status: {{ ucfirst($withdrawal->status) }}</p>
                    </div>
                    @empty
                    <p class="text-gray-500">No withdrawal records yet.</p>
                    @endforelse
                </div>
            </section>
        </main>
